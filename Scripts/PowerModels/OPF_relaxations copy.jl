...
data_1803 = _PM.parse_file(data_file)

ICs = [(i, data_1803["bus"]["$(branch["f_bus"])"]["area"], data_1803["bus"]["$(branch["t_bus"])"]["area"]) for (i, branch) in data_1803["branch"] if data_1803["bus"]["$(branch["f_bus"])"]["area"] !== data_1803["bus"]["$(branch["t_bus"])"]["area"]]

Candidate_SVCs = sort!([(gen["qg"], i, gen["fuel"]) for (i, gen) in data_1803["gen"] if (gen["pg"]==0 && gen["qg"]!==0)])
